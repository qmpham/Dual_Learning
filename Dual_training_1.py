        ...
        #Dual update
		    ...
		    #sampling
			    ...
			    tmp = []
			        ...
			        if len(ss)>0:
                            ...
                            sample, _ = f_sampler_en_fr(x_current, numb_samplings, 200)		    
                            tmp = []
                            for xs in x_en[jj]:
                                tmp.append([xs])
		    ...
		    u1 = time.time()
                ...
                #Time for dual ascent update: average over batch then over samples
                    ...
                    backward_en_fr = f_log_probs_fr_en(numpy.reshape(s_mid_fr,(1,s_mid_fr.shape[0],s_mid_fr.shape[1])), \
                                                 ...
                                                 s_source_en_mask)
                    try:
                        reward_en_fr = ( forward_en_fr * alpha + backward_en_fr * (1-alpha) - dist_BCOW_en_fr * dist_scale - length_constraint_en_fr * length_constraint_scale ) * reward_scale
                    except:
                        ipdb.set_trace()
		    ...
		    if len(x_current)>0:
		        ...
		        if sampling == "beam_search":
		            ...
		            sample, _ = f_sampler_fr_en(x_current, numb_samplings , 200)
                            tmp = []
                            for xs in x_fr[jj]:
                                tmp.append([xs])
                            sample = [list(numpy.trim_zeros(item)) for item in zip(*sample)]
