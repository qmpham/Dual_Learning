...
def train(dim_word = 512,  # word vector dimensionality
    ...
    if use_second_update:
        if optimizers_ is not None:
            ...
            f_dual_grad_shared_en_fr, f_dual_update_en_fr = dual_second_ascent(sample_en_fr, Q_value_en_fr, lr_forward, lr_backward, alp, tparams_en_fr,\
                                                                                             tparams_fr_en, grad_en_fr,\
                                                                                             grad_ce_fr_en, inps_en_fr, inps_fr_en,\
                                                                                             reward_en_fr, avg_reward_en_fr, "en", "fr")
            f_dual_grad_shared_fr_en, f_dual_update_fr_en = dual_second_ascent(sample_fr_en, Q_value_fr_en, lr_forward, lr_backward, alp, tparams_fr_en,\
                                                                                             tparams_en_fr, grad_fr_en,\
                                                                                             grad_ce_en_fr, inps_fr_en, inps_en_fr,\
                                                                                             reward_fr_en, avg_reward_fr_en, "fr", "en") 
    ...
    for training_progress_en_fr.eidx in xrange(training_progress_en_fr.eidx, max_epochs):  
        ...
        #Dual update
		    ...
		    #sampling
			    ...
			    tmp = []
			        ...
			        if len(ss)>0:
                            ...
                            sample, _ = f_sampler_en_fr(x_current, numb_samplings , 50)		    
                            tmp = []
                            for xs in x_en[jj]:
                                tmp.append([xs])
		    ...
		    u1 = time.time()
                    ...
                    sample_2, _ = f_sampler_with_prefix_en_fr(s_source_en_tmp_sample, 1, 50, s_mid_fr_prefix, init_w)
                    ...
                    forward_en_fr_samplings = f_log_probs_fr(s_mid_fr_sampling, s_mid_fr_sampling_mask)
                    backward_en_fr_sammplings = f_log_probs_fr_en(numpy.reshape(s_mid_fr_sampling,(1,s_mid_fr_sampling.shape[0],s_mid_fr_sampling.shape[1])), \
                                                 s_mid_fr_sampling_mask, \
                                                 numpy.reshape(s_source_en_tmp_sample,(s_source_en_tmp_sample.shape[1],s_source_en_tmp_sample.shape[2])),\
                                                 s_source_en_mask_tmp_sample)
                    forward_en_fr_prefix = f_log_probs_fr(s_mid_fr_prefix, s_mid_fr_prefix_mask)
                    backward_en_fr_prefix = f_log_probs_fr_en(numpy.reshape(s_mid_fr_prefix,(1,s_mid_fr_prefix.shape[0],s_mid_fr_prefix.shape[1])), \
                                                 s_mid_fr_prefix_mask, \
                                                 numpy.reshape(s_source_en_tmp_sample,(s_source_en_tmp_sample.shape[1],s_source_en_tmp_sample.shape[2])),\
                                                 s_source_en_mask_tmp_sample)

                    forward_en_fr = forward_en_fr_samplings - forward_en_fr_prefix
                    backward_en_fr = backward_en_fr_sammplings - backward_en_fr_prefix
                    reward_en_fr = forward_en_fr * alpha + backward_en_fr * (1-alpha)
                    Q_value_en_fr = numpy.zeros((s_mid_fr.shape[0],s_mid_fr.shape[1])).astype("float32")
                ...
                #Time for dual ascent update: average over batch then over samples
                    ...
                    dist_CBOW_en_fr = numpy.array([eval(distance)(word_emb_en, word_emb_fr, worddict_en_r, worddict_fr_r, seq1,seq2) \
                                                 for seq1,seq2 in zip(de_factor(s_source_en_tmp),s_mid_fr_tmp)]).astype("float32")
                    ...
                    backward_en_fr = f_log_probs_fr_en(numpy.reshape(s_mid_fr,(1,s_mid_fr.shape[0],s_mid_fr.shape[1])), \
                                                 ...
                                                 s_source_en_mask)
                    try:
                        reward_en_fr = ( forward_en_fr * alpha + backward_en_fr * (1-alpha) - dist_CBOW_en_fr * dist_scale - length_constraint_en_fr * length_constraint_scale ) * reward_scale
                    except ValueError:
                        ipdb.set_trace()
                #print "time to calculate reward: ", time.time()-u1
                    ...
                    dist_acc_en_fr += numpy.mean(dist_CBOW_en_fr)
		    ...
		    if len(x_current)>0:
		        ...
		        if sampling == "beam_search":
		            ...
		            sample, _ = f_sampler_fr_en(x_current, numb_samplings , 50)
                            tmp = []
                            for xs in x_fr[jj]:
                                tmp.append([xs])
		...
		if len(s_source_fr)>0:
		    #sampling:
                    ...
                    sample_2, _ = f_sampler_with_prefix_fr_en(s_source_fr_tmp_sample, 1, 50, s_mid_en_prefix, init_w)
                    sample_2 = [list(numpy.trim_zeros(item)) for item in zip(*sample_2)]
                        #concatenate sample + prefix
                    cols_n_trg_en = []
                    ...
                    forward_fr_en_samplings = f_log_probs_en(s_mid_en_sampling, s_mid_en_sampling_mask)
                    backward_fr_en_sammplings = f_log_probs_en_fr(numpy.reshape(s_mid_en_sampling,(1,s_mid_en_sampling.shape[0],s_mid_en_sampling.shape[1])), \
                                                 s_mid_en_sampling_mask, \
                                                 numpy.reshape(s_source_fr_tmp_sample,(s_source_fr_tmp_sample.shape[1],s_source_fr_tmp_sample.shape[2])),\
                                                 s_source_fr_mask_tmp_sample)
                    forward_fr_en_prefix = f_log_probs_en(s_mid_en_prefix, s_mid_en_prefix_mask)
                    backward_fr_en_prefix = f_log_probs_en_fr(numpy.reshape(s_mid_en_prefix,(1,s_mid_en_prefix.shape[0],s_mid_en_prefix.shape[1])), \
                                                 s_mid_en_prefix_mask, \
                                                 numpy.reshape(s_source_fr_tmp_sample,(s_source_fr_tmp_sample.shape[1],s_source_fr_tmp_sample.shape[2])),\
                                                 s_source_fr_mask_tmp_sample)

		    forward_fr_en = forward_fr_en_samplings - forward_fr_en_prefix
                    backward_fr_en = backward_fr_en_sammplings - backward_fr_en_prefix
                    reward_fr_en = forward_fr_en * alpha + backward_fr_en * (1-alpha)
                    Q_value_fr_en = numpy.zeros((s_mid_en.shape[0],s_mid_en.shape[1])).astype("float32")
                    ...
                    dist_CBOW_fr_en = numpy.array([eval(distance)(word_emb_en, word_emb_fr, worddict_en_r, worddict_fr_r, seq1,seq2) \
                                                 for seq1,seq2 in zip(s_mid_en_tmp,de_factor(s_source_fr_tmp))]).astype("float32")
                ...
                #Time for dual ascent update: average over batch then over samples
                    try:
                        reward_fr_en = ( forward_fr_en * alpha\
                              + backward_fr_en * (1-alpha) - dist_CBOW_fr_en * dist_scale - length_constraint_fr_en * length_constraint_scale ) * reward_scale
                    except ValueError:
                        ipdb.set_trace()
                #print "time to calculate reward: ", time.time() - u1                                                                        
                    u1 = time.time()
                    ...
                    dist_acc_fr_en += numpy.mean(dist_CBOW_fr_en)
        ...
        # test on validation data:
            if numpy.mod(training_progress_en_fr.uidx, validFreq) == 0 or training_progress_en_fr.uidx == 1:
                ...
                if external_validation_script_en_fr is not None:
                    ...
                    print 'Done'
                    p_validation_en_fr = Popen([external_validation_script_en_fr, args_en_fr_1, args_en_fr_2])
                ...
                if external_validation_script_fr_en is not None:
                    ...
                    print 'Done'
                    p_validation_fr_en = Popen([external_validation_script_fr_en, args_fr_en_1, args_fr_en_2])
